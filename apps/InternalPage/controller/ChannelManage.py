...
def select_channel():
    ...
    if city and not channel:
        sql = f'select distinct(source_name),service_type,user_name,city,company_id,details_key,city_name,FROM_UNIXTIME(ctime,"%Y-%m-%d") as ctime' \
              f' from zhuge_dm.city_source where is_dock=1 and city_name="{city}"'
